...
app.layout = html.Div([
    ...
    dcc.Slider(
        id="slider",  # 1800 - 2100 add text to tell user to choose year
        min=1800,
        max=2100,
        value=2020,
...
def update_slider(data_selected):
    if data_selected == 'World population':
        ...
        value = 2020
        step = 1
        marks = {i: '{}'.format(i) for i in range(1800, 2041, 40)}
    elif data_selected == 'Military Expenditure (% of GDP)':  # 1960-2018
        minimum = 1960
        maximum = 2018
        value = 1960
        step = 1
        marks = {i: '{}'.format(i) for i in range(1960, 2019, 40)}
...
def update_graph(data_selected, year_selected):
    ...
    fig = px.choropleth(
        ...
        scope="world",
        hover_name=f"{year_selected}",  # column to add to hover information
